...
class Static_algo:
    ...
    def serve_sequence(self, sequence, type, max_served_events=None):
        ...
        print(f"Event queue counts {len(event_seq)} events with a total of {param.size_dataset} access-requests,"
              f" now serving maximum of {max_served_events} events")
        print("Starting to work through event queue")
        accessed_items = 0
        if self.debug:
            with open("serv_cap_test.txt", "at") as f:
                f.write(f"Static: Server capacity capped at {self.circle.server_capacity}\n\n")
        num_acc_constraint = max_served_events
        if not max_served_events:
            print("Serving whole sequence, no constraint!")
        ...
        while not event_seq.is_empty():
            ...
            if curr_event.get_type() == "access":
                ...
                self.check_insertion_need(curr_event)  # insert item if neede
                if not au.lookup(circle=self.circle, debug=self.debug,
                                 item_id=curr_event.item_id, server_id=curr_event.inserting_server):
                    au.search_update_queue(circle=self.circle, debug=self.debug,
                                           event_seq=event_seq, curr_event=curr_event)
                ...
                if not param.delete_items:
                    continue
                if self.debug:
                    print("Deleting item " + str(curr_event.item_id))
                    with open("event_track_static.txt", "at") as f:
                        f.write(f"Ev. {curr_event.id} deleted item {curr_event.item_id}, n_items={len(self.circle.present_item_ids)}\n")
            ...
            if self.circle.serv_ins_del:
                ...
                if not self.random_adjustment:
                    if self.circle.check_server_capacity_adj_for_item_ins_del(
                        ...
                        if self.debug:
                            with open("serv_cap_test.txt", "at") as f:
                                f.write(f"Event {curr_event.id} processed, timestamp: {curr_event.timestamp}\n\n")
                        self.deleted_items = 0
                        self.inserts_after_init = 0
                        self.reinserted_items = 0
