...
class Server:
    ...
    def insert(self, item_id, init=False, buffer=False):
        ...
        :param buffer: whether we need an extra slot (shouldn't be)
...
class Circle:
    def __init__(self, id, n_servers, m_items, serv_ins_del=True):
        self.id = id
        ...
        self.checking_failure = False
        self.failed_at = -99
        self.traditional_algo_failed = False
        self.serv_ins_del = serv_ins_del

        if param.exp_type == "ctr":
            if not self.serv_ins_del:
                raise Exception('CTR-exp needs to have server insertion/deletion')
    ...
    def stop_checking_failure(self):
        self.checking_failure = False
        ...
        for i in range (0, len(self.present_server_ids)):
            ...
            if parent.id >= self.get_item_position(item_id):
                ...
                if not parent.is_full():
                    ...
                    if self.checking_failure:
                        if not self.traditional_algo_failed:
                            print(f"!!! Traditional algo FAILED after fitting in {len(self.present_item_ids)} items !!!")
                            self.traditional_algo_failed = True
                            self.failed_at = len(self.present_item_ids)
            ...
            # special case where we are trying to insert in a deleted server at end of circle
                ...
                #raise Exception("Shouldn t happen in simple version")
                if init:
                    if self.debug:
    ...
    def insert_new_server(self):
        ...
        for i in range(0, len(self.present_server_ids)+1):      # iterate from 0 to current n_servers+1
            if i not in self.present_server_ids:
                if param.addit_augm[0] == "*":
                    new_s = Server(id=i, capacity=math.ceil(
                        (len(self.present_item_ids) / len(self.present_server_ids)) * 1.25))
                elif param.addit_augm == "inf":
                    if param.exp_type == "fb":
                        new_s = Server(id=i, capacity=16743)
                    elif param.exp_type == "ctr":
                        new_s = Server(id=i, capacity=1029)
                    elif param.exp_type == "temp":
                        new_s = Server(id=i, capacity=param.n_items)
                    else:
                        raise Exception("To determine")
                else:
    ...
    def cap_adj_after_server_ins_del(self):
        ...
        curr_server = self.root
        if param.addit_augm[0] == "*":
            new_cap = math.ceil((len(self.present_item_ids) / len(self.present_server_ids)) * 1.25)
        elif param.addit_augm == "inf":
            if param.exp_type == "ctr":
                new_cap = 1029
            elif param.exp_type == "temp":
                new_cap = param.n_items
            else:
                raise Exception("To determine")
        else:
            new_cap = math.ceil(len(self.present_item_ids) / len(self.present_server_ids)) + int(param.addit_augm)
        if self.debug:
            print(f"Adjusting capacity of all servers, new cap should be: {new_cap} ")
            print(f"{len(self.present_item_ids)} items for {len(self.present_server_ids)} servers")
        with open("serv_cap_test.txt", "at") as f:
            f.write(f"After server insertion/deletion: Adjusting all server's capacity to be {new_cap}\n")
            f.write(f"({len(self.present_item_ids)} items for {len(self.present_server_ids)} servers)\n\n")
        for i in range(len(self.present_server_ids)):       # do for all servers
            diff = new_cap - curr_server.capacity
