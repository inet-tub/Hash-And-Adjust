...
def check_dataset():
    if param.exp_type == "temp":
        ...
        if not os.path.exists("data/caida_grouped.csv.json"):
            raise Exception("Could not find caida-dataset")
    else:
...
def generate_serv_ins_del_ctr():
    ...
    server_insert_timestamps = poisson.rvs(mu=param.serv_ins_freq,
                                                size=int(
                                                    num_events / param.serv_ins_freq + num_events / param.serv_ins_freq * 0.1)).tolist()
    server_deletion_timestamps = poisson.rvs(mu=param.serv_del_freq,
                                                  size=int(
                                                      num_events / param.serv_del_freq + num_events / param.serv_del_freq * 0.1)).tolist()
    saving_dict = {}
